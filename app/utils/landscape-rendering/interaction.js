...
export default Object.extend(Evented, AlertifyHandler, {
  ...
  onMouseWheelStart(evt) {
    ...
    this.get('camera').position.addVectors(
      this.get('camera').position,
    ...
    if (intersectedViewObj) {
      ...
      if (emberModelName === 'application') {
        if (emberModel.get('components').get('length') === 0) {
          ...
          const message = `Sorry, there is no information for application <b>${emberModel.get('name')}</b> available.`;
