...
export default function applyKlayLayout(landscape) {
      ...
      systems.forEach((system) => {
        ...
        if (system.get('opened')) {
          const minWidth = Math.max(
            2.5 * DEFAULT_WIDTH * CONVERT_TO_KIELER_FACTOR,
            (calculateRequiredLabelLength(system.get('name'), SYSTEM_LABEL_HEIGHT) + (PADDING * 6.0)) * CONVERT_TO_KIELER_FACTOR
          );
  ...
  function setAbsolutePositionForNode(child, parent) {
    ...
    const { padding } = parent.get('kielerGraphReference');
  ...
  function createEdgeBetweenSourceTarget(sourceApplication, targetApplication) {
    ...
    const edge = createEdgeHelper(sourceApplication, port1, targetApplication, port2);
    return edge;

    // ---------------------------inner functions
    function createSourcePortIfNotExisting(sourceDrawnode) {
      const ports = sourceDrawnode.get('sourcePorts');
      ...
      const id = sourceDrawnode.get('id');

      const maybePort = ports[id];

      if (maybePort == null) {
        const { length } = Object.keys(ports);
    ...
    function createTargetPortIfNotExisting(targetDrawnode) {
      const ports = targetDrawnode.get('targetPorts');
      ...
      const id = targetDrawnode.get('id');
  ...
  function createEdgeHelper(sourceDrawnode, port1, targetDrawnode, port2) {
    ...
    function lookForExistingEdge(sourceDrawnode, id) {
      const { edges } = sourceDrawnode.get('kielerGraphReference');
      const { length } = edges;
    ...
    applicationCommunications.forEach((applicationcommunication) => {
      ...
      kielerEdgeReferences.forEach((edge) => {
        if (edge != null) {
          ...
          const parentNode = getRightParent(
            applicationcommunication.get('sourceApplication'),
          ...
          var edgeOffset = {
            bottom: 0.0, left: 0.0, right: 0.0, top: 0.0
          };
          ...
          if (parentNode != null) {
            ...
            if (isDescendant(edge.targetNode, edge.sourceNode)) {
              ...
              const sourceInsets = edge.sourceNode.get('kielerGraphReference').padding;

              sourcePoint.x -= sourceInsets.left;
              sourcePoint.y -= sourceInsets.top;

              const nestedGraph = edge.sourceNode.get('kielerGraphReference');
            ...
            points.forEach((point) => {
              ...
              // KIELER has inverted Y coords
              resultPoint.y = ((point.y * -1) + pOffsetY) / CONVERT_TO_KIELER_FACTOR;
