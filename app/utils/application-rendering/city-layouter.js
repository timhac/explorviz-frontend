...
export default function applyCityLayout(application) {
  ...
  */
  ...
  function layoutGeneric(children) {
    ...
    rootSegment.width += labelInsetSpace;
  ...
  function createRootSegment(children) {
    ...
    children.forEach((child) => {
      worstCaseWidth += (child.get('width') + (insetSpace * 2));
      worstCaseHeight += (child.get('depth') + (insetSpace * 2));
    });
    ...
    cumulatedClazzCommunications.forEach((clazzCommunication) => {
      if (!clazzCommunication.get('hidden')) {
        ...
        if (sourceClazz !== null && targetClazz !== null) {
          ...
          clazzCommunication.set('startPoint', new THREE.Vector3(
            sourceClazz.get('positionX') + (sourceClazz.get('width') / 2.0),
            sourceClazz.get('positionY'),
            sourceClazz.get('positionZ') + (sourceClazz.get('depth') / 2.0)
          ));
          clazzCommunication.set('endPoint', new THREE.Vector3(
            targetClazz.get('positionX') + (targetClazz.get('width') / 2.0),
            targetClazz.get('positionY') + 0.05,
